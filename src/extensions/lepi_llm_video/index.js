...
let serverUrl = 'http://106.14.203.61:9001'
if (location.protocol == "https:") {
    serverUrl = `${location.origin}/chat-api`
}
const proxyHost = 'http://localhost:20110/proxy'
// 最大上下文长度,这里设定-11为保留11条上下文，即和大模型的之前11条互动文本，6问5答
const maxLength = -11
class LepiLLMVideo extends EventEmitter {
    constructor(runtime) {
        super();
        /**
    ...
    getInfo() {
        return {
            ...
            blocks: [
                ...
                */
                {
                    opcode: 'sendVideoMessage',
                    text: formatMessage({
                        id: 'lepi.sendVideoMessage',
                        default: '文生视频： [MESSAGE]',
                    }),
                    blockType: BlockType.COMMAND,
                    arguments: {
                        MESSAGE: {
                            type: ArgumentType.STRING,
                            defaultValue: '一只可爱的小猫弹吉他'
                        }
                    }
                },
                // {
    ...
    sendVideoMessage(args, util) {
        ...
        if (true) {
            return new Promise(resolve => {
                axios.post(encodeURI(serverUrl + "/test"), sendData).then(res => {
                    this.response = res.data.data.result_id
    ...
    async requestSuccess(args, util) {
        if (this.result_id) {
            let res = await axios.post(serverUrl + '/getVideo', {
                result_id: this.result_id,
            });
            console.log(res.data)
            if (res.data && res.data.data && res.data.data.videoUrl && res.data.data.videoUrl.indexOf('http') >= 0) {
                return true
            } else {
                return false
            }
    ...
    async getVideoUrl(args, util) {
        if (this.result_id) {
            let res = await axios.post(serverUrl + '/getVideo', {
                result_id: this.result_id,
            });
            console.log(res.data)
            if (res.data && res.data.data && res.data.data.videoUrl) {
