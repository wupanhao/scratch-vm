    ...
    async _initializeSession() {
        ...
        try {
            ...
            if (this.runtime.ros && this.runtime.ros.isConnected()) {
                let res = await this.runtime.ros.proxyPost(encodeURI(this.mcpServerUrl), JSON.stringify({ method: "CUSTOM_POST", headers: this._getBaseHeaders() }), JSON.stringify(body))
                console.log(res)
                response = JSON.parse(res)
            } else {
                response = await axios.post(`${this.mcpServerUrl}`, body, {
            ...
            if (error.response) {
                ...
                if (error.response.status === 401 || error.response.status === 403) {
                    ...
                    if (this.runtime.ros && this.runtime.ros.isConnected()) {
                        let res = await this.runtime.ros.proxyPost(encodeURI(this.mcpServerUrl), JSON.stringify({ method: "CUSTOM_POST", headers: this._getBaseHeaders() }), JSON.stringify(body))
                        console.log(res)
                        retryResponse = JSON.parse(res)
                    } else {
                        retryResponse = await axios.post(`${this.mcpServerUrl}`, body, {
                            headers: this._getBaseHeaders(),
                            timeout: 30000
                        });
                    }
