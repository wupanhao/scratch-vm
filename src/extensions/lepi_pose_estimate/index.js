...
class LepiPoseEstimate extends EventEmitter {
    constructor(runtime) {
        super();
        /**
    ...
    detectPose(args, util) {
        return new Promise((resolve) => {
            this.runtime.ros.detectPose().then(data => {
                this.result = JSON.parse(data)
                resolve(this.result.keypoints.length)
            })
        })
    }
    detectedPose() {
        return this.result.keypoints && this.result.keypoints.length >= 1
    }

    detectPercentage(args, util) {
        if (this.result.keypoints && this.result.keypoints.length > 0) {
            let sum = this.result.keypoints.reduce((acc, item) => acc + item[3], 0)
            let percentage = sum / this.result.keypoints.length
            return percentage
        } else {
            return 0
        }
    ...
    poseKeypoints(args, util) {
        ...
        if (this.result.keypoints.length >= 1) {
            return this.result.keypoints[id][axis]
        }
        return 0
    }
}
...
module.exports = LepiPoseEstimate;